...
class M_transaksi extends CI_Model
{
    ...
    {
        $this->db->select('id_trans, no_hp_cus, id_karyawan, trans_masuk, trans_ambil, pembayaran, status_pembayaran, total_harga');
    ...
    public function updatePembayaranTransaksi($id_trans, $data)
    {
        $this->db->where('id_trans', $id_trans);
        if (!$this->db->update('transaksi', $data)) {
            log_message('error', 'Gagal memperbarui status pembayaran transaksi: ' . $this->db->last_query());
            return false;
        }
        return true;
    }
